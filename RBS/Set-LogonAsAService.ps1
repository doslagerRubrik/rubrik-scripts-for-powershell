...
Invoke-Command -ComputerName $computerName -Script {
    ...
    try {
        ...
        Write-Host "Exporting Local Policy from $ComputerName to temp file"
        ...
        Write-Host "Importing Local Policy on $ComputerName with $UserName added to SeServiceLoginRight"
        secedit /import /db $secedt /cfg $import | out-null
        Write-Host "Configuring Local Policy on $ComputerName"
        secedit /configure /db $secedt | out-null
        Write-Host "Refreshing Group Policy on $ComputerName to apply updates to Local Policy"
...
} -ArgumentList ($username, $computerName)
